...
const response = await fetchUtils.fetchJson(`${API_URL}/role`);
const validateZipCode = regex(
  /^\d{2}-\d{3}$/,
  "Kod pocztowy musi być w formacie XX-XXX",
);
const validateNameUnique = async (enteredName: string, id: string) => {
...
const UsernameValidator = async (value: string, fields: Fields) => {
  if (!value) {
